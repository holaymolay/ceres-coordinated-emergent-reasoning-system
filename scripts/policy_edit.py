#!/usr/bin/env python3
"""Helper for editing and validating CERES macro policy settings."""
...
def ensure_proposed(current_path: Path, proposed_path: Path) -> bool:
    ...
    if not current_path.exists():
        raise SystemExit(f"Current policy not found: {current_path}")
...
def run_policy_guard(
    ...
    if not guard.exists():
        raise SystemExit(f"Policy guard not found: {guard}")
    cmd = [
        sys.executable,
        str(guard),
        "--current",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Edit and validate ceres.policy.yaml safely.")
    parser.add_argument("--current", default="ceres.policy.yaml")
    parser.add_argument("--proposed", default="ceres.policy.proposed.yaml")
    parser.add_argument("--open", action="store_true", help="Open the proposed policy in $EDITOR.")
    parser.add_argument("--editor", default=None, help="Override $EDITOR for --open.")
    parser.add_argument("--apply", action="store_true", help="Apply proposed policy via policy guard.")
    ...
    parser.add_argument("--json", action="store_true", help="Emit policy guard output as JSON.")
    ...
    if created:
        print(f"Created proposed policy: {proposed_path}")

    if args.open:
        open_in_editor(proposed_path, args.editor)

    code = run_policy_guard(
    ...
    if not args.apply:
        print("Review proposed policy and re-run with --apply to commit changes.")
